...
import type {
  ...
  PitchInteger,
} from "jsr:@musodojo/music-theory-data@^1.1.0";
...
const DEFAULT_ROOT_PITCH_INTEGER: PitchInteger = 0;
const DEFAULT_ROOT_NOTE_NAME: string = "C";

const DEFAULT_NOTE_SEQUENCE: PitchInteger[] = [0, 2, 4, 5, 7, 9, 11];
...
class NoteExerciseLooper extends HTMLElement {
  ...
  #rootPitchInteger: PitchInteger = DEFAULT_ROOT_PITCH_INTEGER;
  ...
  constructor() {
    ...
    const audioSpriteUrl = new URL(
      "../assets/nylon-guitar-audio-sprite.ogg",
      import.meta.url,
    ).href;
  ...
  #addEventListeners() {
    ...
    this.#enharmonicNoteSelector!.addEventListener(
      ...
      ((e: CustomEvent<EnharmonicNoteSelectedEventDetail>) => {
        this.#rootPitchInteger = e.detail.pitchInteger;
    ...
    this.#noteSequenceSelector!.addEventListener(
      ...
      ((e: CustomEvent<NoteSequenceSelectedEventDetail>) => {
        this.#currentSequence = e.detail.noteSequenceTheme.sequence;
  ...
  #loadState() {
    ...
    this.#rootPitchInteger = state.rootPitchInteger ??
  ...
  #generateExercise() {
    ...
    for (let octave = 1; octave < this.#numOctaves; octave++) {
      this.#currentSequence.forEach((interval) => {
        this.#exerciseNotes.push(
          rootNoteOctaveMidi + this.#rootPitchInteger + octave * 12 + interval,
        );
      });
    }
    ...
    if (this.#numOctaves > 0) {
      ...
      this.#exerciseNotes.push(
        rootNoteOctaveMidi + this.#rootPitchInteger + this.#numOctaves * 12,
      );
      ...
      if (this.#extraNotes > 0) {
        const baseNoteForExtraOctave = rootNoteOctaveMidi +
          this.#rootPitchInteger + this.#numOctaves * 12;
